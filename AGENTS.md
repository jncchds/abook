...
- `src/abook-ui/src/pages/GlobalSettings.tsx` — Global settings page (MCP Access, LLM config user-default scope, Ollama model pull). Route: `/settings`.
...
- `src/abook-ui/src/App.tsx` — Two-layout routing: `BookListLayout` for `/`, `/settings`, `/presets`, `/admin/users`; `BookLayout` for `/books/:bookId/*` sub-routes (param is `bookId`)
...
- **`[Authorize]` on all API controllers**: All 9 write-facing controllers now require authentication. Content controllers (`CharactersController`, `PlotThreadsController`, `StoryBibleController`, `ChaptersController`, `PlanningPhasesController`, `ConfigurationController`, `MessagesController`, `BooksController`, `AgentController`) require `[Authorize]`. Each content controller has a `CheckOwnershipAsync(bookId)` helper that verifies `book.UserId == null || book.UserId == CurrentUserId`.
...
- **`submittingAnswer` state**: `BookContext` exposes `submittingAnswer: bool`; `handleAnswer` sets it true before `postAnswer` and false in `finally`. `ChatPage` disables the Send Answer button while `submittingAnswer` is true.
...
- **Export filename transliteration**: `SafeFilename` in `ExportController.cs` uses a `TranslitMap` dictionary for Cyrillic + Greek → Latin + NFD normalization to strip Latin diacritics before stripping non-ASCII. Ensures filenames like `vojna-i-mir.html` instead of `.html` for non-ASCII book titles.
...
- **All export logic is server-side**: HTML, FB2, EPUB, and metadata are all generated by `ExportController`. UI uses `window.location.href = /api/books/{id}/export?format=...` for all formats.
...
- **Characters/PlotThreads archive + per-item history UI**: `Characters.tsx` and `PlotThreads.tsx` have: (1) 🗄 archive button (replaces delete); (2) "Show archived" collapsible section with ♻ Restore button; (3) 📜 per-item history button that opens an inline version list with ↩ Restore per version; (4) 📜 collection-level history button in `PhaseActionBar` for phase-reset snapshots (existing behaviour). `CharactersController` and `PlotThreadsController` expose: `POST /{id}/archive`, `POST /{id}/unarchive`, `GET /{id}/history`, `GET /{id}/history/{vId}`, `POST /{id}/history/{vId}/restore`.
...
- **Fixed check-edit-check sequence**: `ProcessChapterAsync` in `AgentOrchestrator` runs PreCheck → Write → Check → optional human pause (if `book.HumanAssisted`) → Editor (only if issues found) → final informational Check → Done. Single edit pass only — no looping. `EditorAgent.EditAsync` called with `finalizeStatus: false`; status is forced to Done at the end. Final check is purely informational (no further editing regardless of result).
...
- **`CheckerResult` record**: in `ABook.Agents/CheckerResult.cs` — `record CheckerResult(bool HasIssues, string[] ContinuityIssues, string[] StyleIssues, string Summary)`. Checker prompts output JSON `{hasIssues, continuityIssues, styleIssues, summary}` parsed via `CheckerResultDto`. Editor receives structured issues list instead of raw text. JSON is extracted via `ExtractJson(responseJson, '{', '}')` before parsing to handle LLM preamble text; fallback message is human-readable (not raw JSON).
