...
- **`PlannerAgent` [ASK:] streaming interruption**: Replaced the old "post-generation Q&A" pattern (generate JSON + ask questions from a `## Questions` section) with mid-stream interruption. New `StreamWithQuestionsAsync` in `AgentBase` scans streaming tokens for complete `[ASK: question]` markers using a compiled regex. When detected: the stream is cancelled via a local CTS; the marker is stripped from output (never sent to SignalR or stored in text); the question is asked via `AskUserAndWaitAsync`; the answer + Q are appended to `qaContext`; the partial assistant text and the answer are added back to `ChatHistory`; the LLM call restarts. The LLM therefore generates the final JSON *with the answer already in context*. A `HoldBack` window of 8 chars suppresses partial `[ASK:` prefixes from being forwarded to SignalR until the tag resolves.
- **`StreamWithQuestionsAsync` in `AgentBase`**: New protected helper (replaces per-use combinations of `StreamResponseAsync` + `AskQuestionsAsync`). Signature: `(kernel, history, bookId, chapterId, role, qaContext, ct, maxQuestions=3)`. When `maxQuestions` reached, injects a user message telling the LLM to stop asking. Accumulates clean text, emits token stats/usage records, supports debug logging — same as `StreamResponseAsync`. `StreamResponseAsync` still exists unchanged for other agents.
- **`PlannerAgent.PlanAsync` `skipCompletedPhases`**: New `bool skipCompletedPhases = false` parameter. When `true`, each phase checks DB data existence before calling the LLM: Phase 1 skips if `GetStoryBibleAsync != null`; Phase 2 skips if characters exist; Phase 3 skips if plot threads exist; Phase 4 skips if chapters exist (returns early). All phase local variables (`bible`, `characters`, `threads`) are still populated from DB when skipped so subsequent phases have context to reference.
- **`PlannerAgent` old Q&A helpers removed**: `ParseQuestionsSection` and `AskQuestionsAsync` methods deleted; `## Questions` section parsing is no longer used.
- **`ContinueWorkflowAsync` planning gap-filling**: Before the chapter writing loop, checks whether Story Bible, Characters, Plot Threads, and Chapters exist. If any are missing, calls `PlanAsync(bookId, skipCompletedPhases: true, ct)` to fill only the gaps, emitting a `WorkflowProgress` message listing the gaps. The hard "No chapters — run planner first" exception is replaced by this auto-fill behaviour. The `TryStartRun` lock role changed from `Writer` to `Planner` (since planning may run first).
- **"✔ Mark Done" button**: Appears in the chapter header for any chapter with `status !== 'Done'` when not running. Calls `PUT /api/books/{id}/chapters/{chapterId}` with `status: 'Done'` via existing `updateChapter` API. Updates both `activeChapter` and the sidebar chapter list. CSS class `btn-mark-done` (green outline, fills on hover) added to `index.css`.
