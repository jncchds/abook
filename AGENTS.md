...
- `src/abook-ui/src/utils/bookHtmlExport.ts` — Retained but no longer imported by any component. Contains `generateBookHtml`, `generateBookMetadataHtml`, `downloadBookMetadataAsHtml` and the `TRANSLIT_MAP` / `transliterate` helpers. All four export formats (HTML, FB2, EPUB, metadata) are now generated and downloaded via `ExportController` server-side.
...
- **Export filename transliteration**: `SafeFilename` in `ExportController.cs` uses a `TranslitMap` dictionary for Cyrillic + Greek → Latin + NFD normalization to strip Latin diacritics before stripping non-ASCII. Client-side `safeFilename` in `bookHtmlExport.ts` has an equivalent `TRANSLIT_MAP`. Ensures filenames like `vojna-i-mir.html` instead of `.html` for non-ASCII book titles.
...
- **All export logic is server-side**: HTML, FB2, EPUB, and metadata are all generated by `ExportController`. UI uses `window.location.href = /api/books/{id}/export?format=...` for all formats. `bookHtmlExport.ts` still exists in the source tree but is no longer imported by any component — it can be deleted in a future cleanup.
