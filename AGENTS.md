...
- **LlmConfiguration**: Id, BookId (nullable, FK → Book), **UserId (nullable, FK → AppUser)**, Provider (Ollama/OpenAI/OpenAICompatible/GoogleAIStudio), ModelName, Endpoint, ApiKey (nullable), EmbeddingModelName (nullable)
...
- **LLM provider is pluggable** — abstracted behind `ILlmProviderFactory`, configured per-book or globally; supported: Ollama, OpenAI, OpenAICompatible, GoogleAIStudio.
...
- **Checker → mechanical patch apply**: `ContinuityCheckerAgent.CheckAsync` accepts optional `humanNotes` param appended to the user message; zone-structured prompts (Intra-chapter Consistency, Cross-chapter Continuity, Grammar & Mechanics, Repetition & Echoed Language) categorize issues as `continuity`, `grammar`, `repetition`, `style`, or `rewrite`. Verbatim patches carry originalText (min 15 chars surrounding context), replacementText, and a required 1-indexed position line-number hint. `EditorAgent.EditAsync` dispatches to `ApplyPatchesAsync` when checker issues exist: patches located via offset-aware normalization (`NormalizeForMatch` unifies CRLF→LF before trimming trailing whitespace per line; `BuildOffsetMapping` walks trimmed lines building a parallel normalized→original index so offsets translate correctly even with \r\n shrinkage), applied end-first to preserve offsets, position used as disambiguator for duplicate matches. Post-apply verification (`VerifyPatchesApplied`) re-runs match logic on patched content to catch phantom artifacts — residual originalText left by offset misalignment is logged and reported in feedback. `EditWithLlmAsync` handles manual edits / MCP tools without checker input. Feedback grouped by type shows original → replacement + description per fix; unapplied patches reported factually with skip reason.
...
- **LlmProvider.OpenAI endpoint support**: when `Endpoint` is non-empty, uses the URI-based `OpenAIChatCompletionService` overload so any OpenAI-compatible API (Groq, Together, etc.) works. When `Endpoint` is empty, uses the standard apiKey-only overload (real OpenAI API). Same logic applies to `CreateEmbeddingGeneration` and `CreateKernel`. Note: for endpoints that return thinking/reasoning tokens in non-standard SSE fields (e.g. Nemotron on OpenRouter), use `LlmProvider.OpenAICompatible` instead which bypasses MEAI's SSE parser entirely.
- **LlmProvider.OpenAICompatible**: custom `OpenRouterHttpChatClient` parses SSE directly for any OpenAI-compatible endpoint (LMStudio, OpenRouter, Groq, Together, etc.). Surfaces reasoning tokens via MEAI's `AdditionalProperties["ReasoningContent"]` channel so AgentBase can extract them separately from content — critical for planning agents that output structured JSON where mixed thinking prose would corrupt parsing. Model listing uses the same `/models` endpoint as OpenAI.
...
- **Azure OpenAI removed**: Not supported in the codebase (`LlmProvider` enum has only Ollama, OpenAI, OpenAICompatible, GoogleAIStudio). Azure endpoints are functionally identical to the OpenAI provider — users should use OpenAI with a custom endpoint. Removed from README docs.
...
- **`CheckerResult` record**: in `ABook.Agents/CheckerResult.cs` — `record CheckerResult(bool HasIssues, CheckerIssue[] Issues, string Summary)` where each `CheckerIssue(Type, Description, ProposedFix, OriginalText, ReplacementText, Position)` carries a 1-indexed line-number hint for disambiguation. Issue types: `continuity`, `grammar`, `repetition`, `style`, `rewrite`. Verbatim-patch issues (zones 2-4) require originalText with min 15 chars surrounding context; rewrite-type issues (Zone 1 intra-chapter contradictions) carry Problem/CanonicalFact/Location/SuggestedRewrite instead. JSON output parsed via `CheckerResultDto` with optional position field; Editor separates patches from rewrites — applies verbatim patches mechanically first, then runs a creative LLM pass only for rewrite issues.
