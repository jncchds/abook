...
- ~~`src/abook-ui/src/utils/bookHtmlExport.ts`~~ — **Deleted**. All export formats (HTML, FB2, EPUB, metadata) are now generated server-side by `ExportController`.
...
- **Export filename transliteration**: `SafeFilename` in `ExportController.cs` (and the now-deleted client-side equivalent) uses a `TranslitMap` dictionary for Cyrillic → Latin + NFD normalization to strip Latin diacritics before stripping non-ASCII. Ensures filenames like `война-и-мир.html` instead of `.html` for non-ASCII book titles.
...
- **Chapter list page no inline content**: the `Chapters.tsx` list view no longer renders the streaming chapter prose inside cards. Cards show only status badge, POV character, outline, and foreshadowing/payoff notes.
- **All export logic is server-side**: `bookHtmlExport.ts` deleted. HTML, FB2, EPUB, and metadata are all generated by `ExportController`. UI uses `window.location.href = /api/books/{id}/export?format=...` for all formats.
- **Metadata HTML export**: `GET /api/books/{id}/export?format=metadata` — server-side generation via `ExportController.GenerateMetadataHtml`. Accepts characters, plot threads, messages, token usage from the repository. Produces a self-contained themed HTML document (same 6 colour presets, font-size controls) with: Book Information, Chapter Outlines (with POV/foreshadowing/payoff), Story Bible, Characters, Plot Threads, Agent Messages (grouped by chapter), Token Statistics. The "⬇ Download Metadata" sidebar button uses `window.location.href = /api/books/{id}/export?format=metadata`. Filename includes `-metadata` suffix.
