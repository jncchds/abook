...
- **Agent error logging & UI notifications**: `IBookNotifier` gains `NotifyAgentErrorAsync(bookId, agentRole, errorMessage)` → `AgentError` SignalR event. All `catch (Exception ex)` blocks in `AgentOrchestrator` now call this (never on `OperationCanceledException`). `AgentBase` gains `ILoggerFactory` constructor param (all 4 agents updated); creates per-agent logger via `GetType().FullName`. `StreamResponseAsync` wraps streaming in try/catch and logs Error + partial response on failure; warns on empty or very short (<50 char) responses. `GetRagContextAsync` logs a Warning instead of silently swallowing failures. `PlannerAgent.ParseChapterOutlines` failures log the full raw LLM response at Error level and also push an `AgentError` notification to the UI. UI: `useBookHub` exports `setOnAgentError`; `BookDetail` shows a dismissible red `.agent-error-banner` at the top and auto-switches to chat panel on mobile.
...
- **`PlannerAgent`** generates outline directly from book metadata with no up-front question. The default system prompt instructs the LLM to write a thorough, unconstrained outline per chapter (key scenes, emotional beats, foreshadowing, plot advancement) — no sentence-count limit.
