...
- **Sidebar nav dot colors**: `.s-dot` default CSS color changed to `var(--success)` (green) — used for planning phase complete indicators. Chat button explicitly passes `dotColor="var(--warning)"` (yellow) when agent is awaiting user answer. `SidebarBtn` gained an optional `dotColor?: string` prop applied as inline `style`., so a JSON parse failure no longer wipes existing chapters. `ParseChapterOutlines` now extracts the JSON array (first `[` … last `]`) before parsing, making it resilient to LLM preamble/postamble text around the array.
