...
- **Split Planner LLM calls**: Each planning phase (Story Bible, Characters, Plot Threads, Chapter Outlines) now makes two LLM calls: (1) `GatherPhaseQuestionsAsync` — small pre-generation call (via `GetCompletionAsync` in `AgentBase`) asking for up to 3 plot clarification questions; (2) full generation call with all Q&A context injected. Questions are persisted as `AgentMessage` (type=Question) and answers as type=Answer, exactly as before.
- **Auto-mark phases complete**: `PlannerAgent.PlanAsync` marks each phase `Complete` on success via `book.*Status = Complete; await Repo.UpdateAsync(book)`. Both "Plan Only" (`StartPlanningAsync`) and "Write Book" (`StartWorkflowAsync`) flows benefit automatically.
- **`PlannerAgent.PlanAsync` skip flags**: New signature `PlanAsync(bookId, skipStoryBible, skipCharacters, skipPlotThreads, skipChapters, ct)`. Skipped phases load existing data from DB and emit "Skipping: [Phase] (marked complete)" progress. `LoadExistingQaContextAsync` rebuilds Q&A context from existing DB messages for skipped-phase continuations. `AgentOrchestrator.StartPlanningAsync` and `StartWorkflowAsync` call with `ct: ct` named param.
- **`ContinuePlanningAsync`**: `AgentOrchestrator` method reads 4 phase statuses, computes skip flags, calls `PlannerAgent.PlanAsync` skipping completed phases. If all 4 complete, emits "All planning phases are already complete." message.
