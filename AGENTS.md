...
- **Checker → mechanical patch apply**: checker prompts categorize issues as `continuity`, `grammar`, `repetition`, or `style` with verbatim `originalText` (context-rich, min ~20 chars), `replacementText`, and an optional 1-indexed `position` line-number hint. `EditorAgent.EditAsync` dispatches to `ApplyPatchesAsync` when checker issues exist: patches located via IndexOf-first matching with whitespace normalization (`CRLF→LF`, trailing-trim), applied end-first to preserve offsets, position used as a disambiguator only for duplicate matches. `EditWithLlmAsync` handles manual edits / MCP tools without checker input. Feedback message grouped by issue type shows original → replacement + description per fix; unapplied patches reported factually with skip reason.
...
- **Author edits are embedded**: `SearchAsync` only returns chunks whose `ChapterVersionId` is the chapter's *active* version, so a hand-edited chapter used to vanish from RAG — its new version had no chunks. `ChapterIndexer.IndexVersionAsync` (`ABook.Infrastructure/VectorStore`) is the one embedding routine, used by `AgentBase.IndexChapterAsync` and by `ChapterIndexingService` (`ABook.Api/Services`, singleton). `ChaptersController.Update` queues indexing for a content-changing author edit; `ActivateVersion` queues it for any activated version that has no embeddings yet. Queued work runs on a detached DI scope so the save request returns immediately, and bills an `Embedder` `TokenUsageRecord` like an agent run does. `ChapterVersion.HasEmbeddings` is now actually set to `true` on success (it was written as `false` everywhere and never updated), and the chapter version history shows it.
...
- **LlmProvider.GoogleAIStudio (int=5)**: chat *and* embeddings both go through Google's OpenAI-compatible endpoint (`https://generativelanguage.googleapis.com/v1beta/openai`) via the shared `OpenAIClient` pattern — there is no Semantic Kernel Google connector in the path. `response_format: json_schema` works there, including array-root schemas and `["integer","null"]` unions, so the planning agents' schemas need no Gemini-specific rewriting. API key is required and mandatory. Default model suggestions: `gemini-2.5-flash`, `gemini-3.5-flash` etc. Default embedding model: `text-embedding-004`.
...
- **Network timeout on the `OpenAIClient` strategies**: `OpenAIProviderHelpers` sets `OpenAIClientOptions.NetworkTimeout` from `LlmConfiguration.TimeoutMs`, falling back to **300 000 ms** — the same default `OpenAICompatibleProviderStrategy` uses for its own `HttpClient`. The SDK's built-in default is 100 s, which a streaming chapter write or a whole-book continuity pass routinely exceeds; before this was wired up the OpenAI and GoogleAIStudio strategies ignored `TimeoutMs` entirely and a long call died mid-stream as `Retry failed after 4 tries … exceeded the configured timeout of 0:01:40`, failing the run after the chapter had already been written.
- **Reasoning effort on GoogleAIStudio**: `LlmConfiguration.ReasoningEffort` is forwarded as `reasoning_effort` via `ChatCompletionOptions.ReasoningEffortLevel` (requires `#pragma warning disable OPENAI001`). Gemini thinking models spend output tokens before emitting any text, so the Advanced LLM Parameters control is load-bearing for this provider; it was previously accepted in the UI and silently dropped.
...
- **`ReplaceChaptersAsync` in `BookRepository`**: atomically syncs a book's outlines to the given set inside a single PostgreSQL transaction. Called by `PlannerAgent.RunAsync` instead of separate `DeleteChaptersAsync` + per-chapter `AddChapterAsync` loop — prevents partial-save state if a chapter insert fails mid-loop. Chapters matching by `Number` are **updated in place** (prose, status, versions and message links survive a re-plan), missing numbers are inserted, and non-archived chapters absent from the new set are deleted; **archived chapters are never touched**. Returns the persisted rows so `PlannerAgent` can hand real Ids to `AgentOrchestrator`. `IBookRepository` interface gains the method.
...
- **Fixed check-edit sequence**: `ProcessChapterAsync` in `AgentOrchestrator` runs PreCheck → Write → Check → mechanical fixes (only if the checker returned patch-type issues) → optional human pause (if `book.HumanAssisted`) → creative rewrite (only if the checker returned `rewrite` issues **or** the author typed instructions during the pause) → Done. Single edit pass only — no looping. The Editor is called with `finalizeStatus: false`; status is forced to Done at the end. No final informational check — patches are deterministic, any failures reported in chat panel.
- **Human pause sits between the mechanical fixes and the creative rewrite**: the author reads prose that already has the verbatim patches applied, and may edit the chapter, characters, plot threads or outlines before answering (`AgentOrchestrator.AskBeforeRewriteAsync` builds the question). `EditorAgent.RewriteAsync` re-reads the chapter from the DB, so those edits are the base text; the answer is injected as an "Author Instructions (highest priority)" block ahead of the checker's rewrite issues. An empty answer with no rewrite issues skips the LLM call entirely.
- **`EditorAgent` phase split**: `ApplyMechanicalFixesAsync` (patches only, no LLM) and `RewriteAsync` (creative pass) are separate public entry points so the orchestrator can interleave the human pause; `EditAsync` keeps the old single-shot behaviour for the standalone Edit button and MCP tools. `SelectPatches`/`SelectRewrites` split a `CheckerResult` into the two buckets.
...
- **Editing while a run is paused**: `BookContext` exposes `isWaitingForInput` and `canEdit` (`!isRunning || isWaitingForInput`) alongside `isRunning`. Views that hid their edit/archive/clear controls behind `!isRunning` (`Overview`, `Chapters`, `ChapterView`) now use `canEdit`, so nothing is locked while an agent waits for an answer. `ChapterView`'s edit form gained a **Content** textarea — chapter prose is editable from the UI, not just title and outline. `ChatPage` and the chapter edit form show an `.edit-during-pause-hint` banner explaining that the next step reads whatever is saved.
- **`ToggleField` component** (`src/abook-ui/src/components/ToggleField.tsx`): switch-style boolean field (visually-hidden native checkbox + `.toggle-track`/`.toggle-thumb`, label plus optional hint). Replaces the bare native checkboxes for "Human-assisted generation" in `Dashboard`, `BookSettings` and `Overview`.
...
- **Failed-call token accounting**: every LLM call is accounted for, including ones that error, time out, or are cancelled. `AgentBase.RecordUsageAsync(bookId, chapterId, role, promptTokens, completionTokens, endpoint, modelName, failureReason, ct)` is the single write path — it notifies over SignalR (falling back to `CancellationToken.None` when `ct` is already cancelled, so failure rows still push live) then persists a `TokenUsageRecord` with `Failed = failureReason is not null`. Never throws; a DB error is logged at `Warning`. Used by `StreamResponseAsync` (success + both catch blocks), `GetRagContextAsync`, and `IndexChapterAsync`.
- **EF migration `AddTokenUsageFailureReason`**: adds `FailureReason varchar(1000) NULL` to `TokenUsageRecords`. Populated by `AgentBase.DescribeFailure(Exception ex, bool cancelledByUser, int? timeoutMs = null)` — an `internal static` pure helper returning `"Cancelled by user"` (agent's own token signalled), `"Timed out after {n}ms"` (`OperationCanceledException` with the token *not* signalled — this is how `HttpClient.Timeout` surfaces), `"HttpRequestException (HTTP {code} {status}): …"`, or `"{TypeName}: {Message}"`, unwrapping single-inner `AggregateException`s and truncating to 1000 chars with `…`.
- **Cancellation no longer loses stats**: `StreamResponseAsync`'s `catch (OperationCanceledException)` previously rethrew before persisting anything. It now records prompt tokens plus the partial `sb.Length / 4` completion estimate, then rethrows. `IndexChapterAsync` bills only the chunks embedded before a mid-loop failure; `GetRagContextAsync` guards with a `usageRecorded` flag so a post-embedding vector-search failure does not double-record.
- **Token Stats `Status` column** (`TokenStatsPage.tsx`): `—` for successful rows, `❌ Failed` / `⏹ Cancelled` (derived from the reason text) for failed ones, with the full reason rendered as a muted, ellipsised sub-row beneath. Footer totals deliberately still include failed rows (their `~`-prefixed estimates count toward the grand total and per-model subtotals). Counts remain the `chars / 4` estimate — `LlmStreamChunk` carries no usage metadata, so no provider reports real token counts.
...
## Resilient JSON planning agents (Characters / Plot Threads / Chapter Outlines)

- **`PartialJson` (`src/ABook.Agents/PartialJson.cs`)**: recovers usable JSON from a response that was cut short mid-stream. `SalvageArray(raw)` strips reasoning tags (an *unterminated* `<think>` means the stream died while reasoning, so everything from it is dropped) and markdown fences, then walks the array tracking bracket depth and string/escape state, returning the top-level elements that finished plus a synthetic `]`. Candidate `[` positions are probed in order and only arrays **of objects** are accepted, so bracketed prose ("the [requested] list") never masks the payload. `IsComplete(raw)` reports whether the array was actually closed — used to detect a clean stream end on a truncated array (model hit `MaxTokens`).
- **`AgentBase.StreamJsonResponseAsync`**: wraps `StreamResponseAsync` and returns `(Raw, Failure)` instead of throwing. On any error — provider timeout, dropped connection, user Stop — it pulls the accumulated text from `AgentRunStateService.GetStreamBufferContent` so tokens the author already watched stream in are never discarded. `AgentBase.Rethrow` preserves the original stack trace; `AgentBase.SerializeForPrompt` renders saved planning data as camelCase JSON with the relaxed encoder (non-Latin scripts stay readable).
- **Keep-partial-on-failure**: `CharactersAgent`, `PlotThreadsAgent` and `PlannerAgent` each salvage what streamed, **merge** it into the existing set (never replace), write versions + an `"agent-partial"` snapshot, post a `SystemNote` naming how many items were kept, and then rethrow so the run is still marked Failed and the phase stays incomplete. The next run continues from there. Merge keys: `Name` (case-insensitive) for characters and plot threads, `Number` for chapters — `MergeCharacterCardsAsync`, `MergePlotThreadsAsync`, `MergeChapterOutlinesAsync` on `IBookRepository`.
- **Empty or truncated results never wipe data**: all three parsers throw `FormatException` when zero usable items are parsed, and a response that ends without closing the array is treated as a failure (salvage + merge + rethrow) rather than a complete set.
- **`LenientJson` (`src/ABook.Agents/LenientJson.cs`)**: reads planning fields without trusting the model to honour the schema's types. OpenAI-compatible endpoints (LM Studio, Ollama, vLLM) routinely return an integer as `1.0` or `"3"` and a text field as an array or a bare number, all of which make the strict `JsonElement` accessors throw. `Int` accepts integers, non-integral numbers (rounded), numeric strings, and numbers embedded in prose (`"Chapter 4"` → `4`), yielding `null` when nothing is recoverable; `Text` coerces any value kind, flattening arrays to a comma-separated list. Used by all four planning parsers.
- **`PlanningParse` (`src/ABook.Agents/PlanningParse.cs`)**: makes a planning failure self-describing. `OpenArray`/`OpenObject` replace the bare `JsonDocument.Parse` and turn "no JSON", "invalid JSON" and "root was not an object" into messages that name the agent and quote what the model actually returned (whitespace-collapsed, 300-char snippet). `SkipNote` describes one rejected element by index, reason, and its raw JSON; `SkipSummary` explains an all-rejected outcome; `KeptSummary` reports the drops on an otherwise successful run.
- **Per-element resilience**: each array parser wraps one element at a time. A malformed entry — no `name`, or any exception while reading it — is dropped with a reason into a `List<string> skipped` collector rather than aborting the run, so one bad entry no longer costs the author the whole set. On success with a non-empty collector the agent logs a warning and posts a `⚠️` `SystemNote` via `AgentBase.ReportNoteAsync` (a non-fatal counterpart to `ReportErrorAsync` — it writes the note and refreshes messages, but raises no `AgentError` signal). All keep-partial and Story Bible failure notes now append `Reason: {ex.Message}` so the underlying cause reaches the chat instead of being replaced by generic advice.
- **Existing content is fed back to the LLM**: each of the three agents loads its already-saved, non-archived rows and appends a block to the user prompt ("These … are already saved for this book: `[…]` — return the complete, updated set, keeping each item's exact `name`/`number` so it can be matched; items you leave out will be removed"). Chapters additionally carry an `alreadyWritten` flag so the model does not re-premise a chapter whose prose exists.
- **Archived content is invisible to agents**: agents read via `GetCharacterCardsAsync` / `GetPlotThreadsAsync` / `GetChaptersAsync` (all `!IsArchived`), and `DeleteCharacterCardsAsync` / `DeletePlotThreadsAsync` / `ReplaceChaptersAsync` now delete **only non-archived rows** — an agent re-run can no longer destroy archived items (this also protects them during snapshot restores, which reuse the same delete helpers).

## Archived content is display-only (v0.1.22)

The invariant: **an archived chapter, character card, or plot thread exists only to be rendered in the UI's archived sections.** It never reaches a model prompt, never lands in an export, and is never mutated by an agent. Enforced at four layers:

- **Repository reads** — `GetChaptersAsync` / `GetCharacterCardsAsync` / `GetPlotThreadsAsync` filter `!IsArchived` at the DB level, and `BuildAncestorPlanningReferenceAsync` applies the same filter when pulling a sequel's ancestor context. `GetChapterAsync` deliberately does **not** filter: the UI must be able to load an archived chapter to display and restore it.
- **Book loading** — `GetByIdWithActiveDetailsAsync` mirrors `GetByIdWithDetailsAsync` but with a filtered `Include` on `Chapters`. `AgentOrchestrator.RunPlanningPipelineAsync`, `ContinuityCheckerAgent.CheckAsync`, `BookExportService.ExportAsync`, and `PublicController.GetBook` all use the active variant. `GetByIdWithDetailsAsync` returns everything and is reserved for `BooksController.GetById`, which feeds the UI's archived sections.
- **Agent entry guards** — `AgentBase.EnsureNotArchived(chapter)` throws when an agent resolves an archived chapter by id. Applied in `WriterAgent.WriteAsync`, `EditorAgent.EditAsync`, and `ContinuityCheckerAgent.PreWriteCheckAndFixAsync`, i.e. every place an agent turns a `chapterId` into a `Chapter`.
- **API and MCP surfaces** — `AgentController.Write`/`Edit` and the `write_chapter`/`edit_chapter` MCP tools reject an archived target before scheduling the fire-and-forget run, so the caller gets a 400 / `McpException` rather than a run that fails later. The `update_chapter`, `update_character`, and `update_plot_thread` MCP tools refuse to modify archived rows; `get_chapter` refuses to serve archived prose.

**RAG** is already covered independently: `PgvectorVectorStoreService.SearchAsync` joins back to `Chapters` and requires `IsArchived = FALSE`, for both versioned and legacy chunks. Chunks are left in place when a chapter is archived so restoring it brings its embeddings back.

**Exports** filter `!IsArchived` a second time inside `GenerateHtml`, `GenerateFb2`, `GenerateEpub`, and `GenerateMetadataHtml` — defence in depth, and the seam the unit tests pin the invariant at.
