...
- `src/ABook.Api/Controllers/` — `BooksController`, `ChaptersController`, `MessagesController`, `ConfigurationController`, `AgentController`, `AuthController`, `UsersController`, `OllamaController`, `PresetsController`, `ExportController` (thin wrapper over `BookExportService`)
...
- `src/abook-ui/src/pages/Library.tsx` — Public library page (`/library` route, outside the auth guard). Paginated/filterable (author, genre, exact chapter count) list of publicly-visible books; login prompt when non-public mode and unauthenticated.
- `src/abook-ui/src/pages/PublicBookReader.tsx` — Public book reader page (`/library/:bookId` route). Chapter nav panel, markdown content, prev/next buttons, HTML/FB2/EPUB download bar.
...
- `src/abook-ui/src/App.tsx` — Two-layout routing: `BookListLayout` for `/`, `/settings`, `/presets`, `/admin/users`; `BookLayout` for `/books/:bookId/*` sub-routes (param is `bookId`); `/library` and `/library/:bookId` mounted outside the auth guard alongside `/login`; mounts `<PwaUpdatePrompt />`
...
- **Checker → mechanical patch apply**: `ContinuityCheckerAgent.CheckAsync` now accepts optional `humanNotes` param which is appended to the user message; checker prompts require verbatim `originalText`/`replacementText` per issue; `EditorAgent.EditAsync` with checker issues iterates issues and applies patches via `string.IndexOf` (OrdinalIgnoreCase fallback) — no LLM call; removed the Final Check step from `ProcessChapterAsync`; human pause happens BEFORE checker so notes are fed into it
...
- **Non-streaming PreWriteCheck**: `ContinuityCheckerAgent.PreWriteCheckAndFixAsync` uses `GetCompletionAsync` (non-streaming) so its output never leaks as AgentStreaming tokens to the chapter view
