...
- **Token usage statistics** — per-agent prompt and completion token counts displayed in a scrollable collapsible panel and persisted to the database
...
- **Story Bible, Characters & Plot Threads** — automatically generated by the Planner agent (4-phase: Story Bible → Characters → Plot Threads → Chapter Outlines); editable at any time
- **Flexible workflow controls** — **Plan Only** (outline the book and stop, so you can review/edit outlines before writing), **Write Book** (full autonomous pipeline), **Continue** (resume from where you left off), and individual per-chapter agent buttons
...
User creates book (title, premise, genre, target chapters)
  ...
  │   ─ asks author questions at the end of each phase  │
  └─────────────────────────────────────────────────────┘
         │
