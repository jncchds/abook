...
- **Checker → mechanical patch apply** — the Checker flags continuity, grammar, repetition, style, and rewrite issues as structured JSON patches with verbatim original text (min 15 chars surrounding context) and replacement; zone-structured prompts (Intra-chapter Consistency, Cross-chapter Continuity, Grammar & Mechanics, Repetition & Echoed Language) drive focused detection. The Editor applies them mechanically via offset-aware normalization (`NormalizeForMatch` unifies CRLF→LF before trimming trailing whitespace, `BuildOffsetMapping` translates normalized→original positions correctly even with line-ending shrinkage), applied end-first to preserve offsets, position as disambiguator for duplicates; post-apply verification re-runs match logic on patched content to catch phantom artifacts. Feedback grouped by type shows all fields per fix.
